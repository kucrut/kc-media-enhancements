...
class kcmeAttachmentTaxonomies {
	...
	public static function init( $taxonomies ) {
		...
		add_action( 'admin_init', array(__CLASS__, '_save_terms'), 111 );
	}
		...
		foreach ( self::$data['taxonomies'] as $tax ) {
			...
			if ( !empty($tax_terms) )
				foreach ( $tax_terms as $term )
					$html .= "\t<label><input type='checkbox' name='attachments[{$post->ID}][{$tax->name}][]' value='".esc_attr($term->name)."' ".checked(array_key_exists($term->term_id, $att_terms), true, false)." /> {$term->name}</label><br />\n";

			# New terms
			$html .= "\t<input type='text' name='attachments[{$post->ID}][{$tax->name}][]' /></label><br />\n";

			$fields[$tax->name]['label'] = $tax->label;
			$fields[$tax->name]['input'] = 'html';
			$fields[$tax->name]['html']  = $html;
			$fields[$tax->name]['helps'] = sprintf(__( 'Check/uncheck existing %s, or add new one(s), separated by commas.', 'kc-media-enhacements'), $fields[$tax->name]['label'] );
		}
